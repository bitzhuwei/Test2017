...
namespace EMGraphics
{
    ...
    public class NASFile
    {
        ...
        private NASFile(List<dvec3> points, List<Triangle> grids)
        {
            this.points = points;
            this.grids = grids;
        }
        ...
        public static NASFile Load(string filename)
        {
            ...
            using (var reader = new StreamReader(filename))
            {
                ...
                points.Add(new dvec3(0, 0, 0));//点坐标的数组是从1开始的,不是从0开始的,方便
                ...
                do
                {
                    ...
                    if (sss.Substring(1, 5) == "GRID")//观察导出文件的格式,当遇到*号时,开始解读点的坐标,反之就是网格的信息
                    {
                        ...
                        {
                            x = double.Parse(str22[2]);
                            y = double.Parse(str22[3]);
                        }
                        else//GRID*              26182                -6.270781884E-01-4.699668723E-02   26182
                        {
                            if (str22[2].Substring(1, 1) == "-")
                            {
                                x = double.Parse(str22[2].Substring(0, 16));
                            }
                            else
                            {
                                x = double.Parse(str22[2].Substring(0, 15));
                            }

                            y = double.Parse(str22[2].Substring(str22[2].Length - 16));
                        }
                    }
                    else if (sss.Substring(1, 1) == "*")//提取Z坐标
                    {
                        z = double.Parse(str22[str22.Length - 1].Substring(str22[str22.Length - 1].Length - 16));
                        points.Add(new dvec3(x, y, z));
                    }
                    else if (sss.Substring(1, 5) == "CTRIA")
                    {
